...
def discovery(isbn: str | None, reason: MatchReason = MatchReason.GENRE_CATEGORY) -> Observation:
    return Observation(source="beam", source_item_id=isbn or "x", title="Ein Buch",
                       match_reason=reason, isbn=isbn)
...
def test_a_find_in_another_language_is_foreign(db: Store) -> None:
    ...
    assert is_foreign(discovery("9780000000001"), load_settings(), language_finder(db))


def test_a_german_find_is_not(db: Store) -> None:
    db.save_dnb("9783000000001", Record(title="Ein Buch", language="ger"), NOW)

    assert not is_foreign(discovery("9783000000001"), load_settings(), language_finder(db))


@pytest.mark.parametrize("how", ["ohne ISBN", "nie gefragt", "DNB kennt es nicht",
                                 "ohne Sprachangabe"])
def test_unknown_is_never_foreign(db: Store, how: str) -> None:
    ...
    isbn = None if how == "ohne ISBN" else "9789920000002"
    if how == "DNB kennt es nicht":
        db.save_dnb("9789920000002", None, NOW)
    if how == "ohne Sprachangabe":
        db.save_dnb("9789920000002", Record(title="A Book"), NOW)

    assert not is_foreign(discovery(isbn), load_settings(), language_finder(db))
...
def test_a_watchlist_title_is_never_foreign(db: Store) -> None:
    ...
    assert not is_foreign(discovery("9780000000001", MatchReason.WATCHLIST), load_settings(),
                          language_finder(db))
...
def test_the_codes_for_unknown_are_not_foreign(db: Store, code: str) -> None:
    ...
    assert not is_foreign(discovery("9780000000003"), load_settings(), language_finder(db))
...
def test_a_hungarian_isbn_is_foreign_even_without_a_dnb_record(db: Store) -> None:
    ...
    assert is_foreign(discovery("9786155522017"), load_settings(), language_finder(db))
...
def test_an_unknown_group_stays_silent(db: Store) -> None:
    ...
    assert not is_foreign(discovery("9789920000001"), load_settings(), language_finder(db))
...
def test_the_dnb_outranks_the_group(db: Store) -> None:
    ...
    assert not is_foreign(discovery("9789630000001"), load_settings(), language_finder(db))
...
def test_the_group_only_speaks_where_the_dnb_is_silent(db: Store) -> None:
    ...
    assert is_foreign(discovery("9783000000099"), load_settings(), language_finder(db))


def test_a_watchlist_title_stays_whatever_its_isbn_says(db: Store) -> None:
    """Was die Leserin selbst auf die Liste setzt, bleibt dort."""
    assert not is_foreign(
        discovery("9786155522017", MatchReason.WATCHLIST), load_settings(), language_finder(db)
    )
