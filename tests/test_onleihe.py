...
def test_a_title_the_library_dropped_is_no_item() -> None:
    class Gone(StubClient):
        def get(self, url: str, params: dict | None = None) -> str:
            raise NotFound(url)

    assert OnleiheSource(Gone("")).item("373164461") is None
...
def test_the_list_yields_fiction_as_borrowable_finds() -> None:
    ...
    discoveries = parse_list(fixture("recently-returned.html"),
                       OnleiheList("lrMediaList,0-0-0-107-0-0-0-0-0-0-0.html",
                                   "Zuletzt zurückgegeben"))
    title = {f.title for f in discoveries}

    assert {"Der eiserne Sommer", "Der fröhliche Frauenhasser",
            "Rom sehen und nicht sterben"} <= title
    assert not title & {"Organische Chemie", "Linsen, Kichererbsen & Co.", "Peer Gynt"}
    discovery = next(f for f in discoveries if f.title == "Der fröhliche Frauenhasser")
    assert discovery.match_reason is MatchReason.GENRE_CATEGORY
    assert discovery.category == "Zuletzt zurückgegeben"
    ...
    assert discovery.isbn == "9783641088286"
    assert discovery.author == "Colin Cotterill"
    assert discovery.source_item_id == "361212177"
...
def test_the_run_collects_the_lists_next_to_the_watchlist(tmp_path) -> None:
    ...
    source = OnleiheSource(client=client, lists=(
    ...
    discoveries = source.collect(load_settings(), [], context)

    assert discoveries and all(f.category == "Zuletzt zurückgegeben" for f in discoveries)
...
def test_the_lists_are_read_from_the_settings() -> None:
    ...
    source = _build_onleihe("onleihe", {"lists": [
        {"path": "lrMediaList,0-0-0-107-0-0-0-0-0-0-0.html", "name": "Zuletzt zurückgegeben"}]},
        StubClient(""))

    assert source.lists == (OnleiheList("lrMediaList,0-0-0-107-0-0-0-0-0-0-0.html",
                                        "Zuletzt zurückgegeben"),)
